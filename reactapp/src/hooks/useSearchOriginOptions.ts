import { searchAirport, searchAirportDummy } from "../services/AmadeusAPIService";
...
export default function useSearchOriginOptions() {
  ...
  async function searchOriginOptions(inputValue: string): Promise<Route[]> {
    try {
      ...
      let response;
      if (process.env.REACT_APP_DEV_MODE === "true") response = await searchAirportDummy(inputValue);
      else response = await searchAirport(inputValue);
      const airports = response.data.data;
      return airports.map((airport: any, index: number) => ({
        value: index,
        label: capitalize(airport.name) + " (" + airport.iataCode + "), " + capitalize(airport.address.countryName),
